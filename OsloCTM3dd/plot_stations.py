...
for i in range(1,len(station_lat)):
    ...
    ax.scatter(ozone_data[0].time.data, hardacre_mmm[i], color='red', label='Hardacre et al. (2015) - mmm')
    ax.fill_between(ozone_data[0].time.data, hardacre_mmm[i]*4/3., hardacre_mmm[i]*2/3., color='red', alpha=0.1)
    ax.scatter(ozone_data[0].time.data, hardacre_obs[i], color='black', marker='v', label='Hardacre et al. (2015) - obs')
...
fig2.axes[4].legend(bbox_to_anchor=(0.35, -0.35), loc=8, borderaxespad=0.,ncol=4)
...
for i in range(1,len(station_lat)):
    ...
    ((ozone_data[0]/grampy).sel(lat=station_lat[i],lon=station_lon[i], method='nearest')-hardacre_obs[i]).plot(ax=ax, zorder=2, ls='--',
                                                                                                          label='$\mathrm{OsloCTM3_{Wesely}}$-$\mathrm{Hardacre_{obs}}$', color='blue')
    ((cases_mean).sel(lat=station_lat[i],lon=station_lon[i], method='nearest')-hardacre_obs[i]).plot(ax=ax, zorder=2, ls='--',
                                                                             label='$\mathrm{OsloCTM3_{<emep>}}$-$\mathrm{Hardacre_{obs}}$', color='orange')
    ...
    #                                                                         label='$\mathrm{OsloCTM3_{<emep>}}$-$\mathrm{Hardacre_{mmm}}$', color='orange')
    ax.set_title(hardacre_labels[i])
    ...
    #chi2_emep = (((((cases_mean).sel(lat=station_lat[i],lon=station_lon[i], method='nearest')-hardacre_obs[i])**2/(((cases_std).sel(lat=station_lat[i],lon=station_lon[i], method='nearest')-1)**2)).sum(dim='time'))/12).data
    chi2_emep = ((((cases_mean).sel(lat=station_lat[i],lon=station_lon[i], method='nearest')-hardacre_obs[i])**2).sum(dim='time')/12).data
    ...
    ax.text(1,0,'$\chi^2/NDF=$%1.2f' % (chi2_emep),
...
fig4.axes[4].legend(bbox_to_anchor=(0.35, -0.35), loc=8, borderaxespad=0.,ncol=4)
