...
def plot_climadiagram(temp, precip, **kwarg):
    ...
    if temp_err is None:
        ...
        temp_max = np.ceil(temp.max()+temp_err.max())
        temp_min = np.floor(temp.min()-temp_err.max())
...
precipitation = (data_svanvik_precip_clim[:'2018'].groupby(['year','month']).sum()).groupby(['month'])
