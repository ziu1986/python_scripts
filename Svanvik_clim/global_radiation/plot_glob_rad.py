...
for iyear, iax, icolor in zip((2018, 2019),(ax31, ax32), ('violet', 'purple')):
    ...
    iax.text(9.25,35, '$>+1\,\sigma$: %3.2f %s' % (test.where(test>1).iloc[:,0].dropna().size/float(test.size)*100, "%"), size='x-large')
    iax.text(9.5,-35, '$<-1\,\sigma$: %3.2f %s' % (test.where(test<-1).iloc[:,0].dropna().size/float(test.size)*100, "%"), size='x-large')
