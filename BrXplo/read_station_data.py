...
def read_station_data_noaa(infile, **kargs):
    ...
    **kargs utc - timeshift wrt UTC
    ...
    import codecs
    # Key words
    start_data = kargs.pop('start_data')
    utc = kargs.pop('utc', 0)
    ...
    for each in lines[start_data:]:
        ...
        # Generate timestamp
        if station=='southpole':
            time.append(dt.datetime.strptime("%s-%s-%s %s" % (col[1], col[2], col[3], col[4]), '%Y-%m-%d %H')-timeshift)
        else:
            time.append(dt.datetime.strptime("%s" % (col[0]+" "+col[1]), '%Y-%m-%d %H:%M')-timeshift)
