...
for ioz in ozone:
    ...
    for ithr in threshold:
        ...
        try:
            ...
            (brazil_sum[(ithr,ioz)]['NPP']*60**2).plot(ax=ax31, color=color, ls=ils, alpha=alpha, linewidth=2.5, label="(%s, %s)" % (ioz,ithr))
            ...
            (100*(brazil_sum[(ithr,ioz)]-brazil_0_ref_sum)/brazil_0_ref_sum)['NPP'].plot(ax=ax32, color=color, ls=ils, alpha=alpha, linewidth=2.5, label="(%s, %s)" % (ioz,ithr))
...
(brazil_0_ref_sum['NPP']*60**2).plot(ax=ax31, color='black', linewidth=3, label='ref')
