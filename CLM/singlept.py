...
From the datm streams files (e.g. datm.streams.txt.CLMCRUNCEP.Precip)
...
After running this script, point to the new CLM surface
dataset using the user_nl_clm file in the case directory.
No initial conditions are available, so set finidat = ' ' in user_nl_clm

In addition:
copy the datm.streams files to the case directory, with the prefix 
'user_', e.g. user_datm.streams.txt.CLMCRUNCEP.Precip.  Change the 
...
River routing can be turned off either in env_run.xml or in user_nl_rtm 
by setting "RTM_MODE" to "NULL". [DLL note: doesn't seem to exist in env_run.xml anymore]

To run on caldera, change the JOB_QUEUE to caldera from regular in 
env_batch.xml
...
tagnum=1
if tagnum == 1: #Single point; point = Iowa
   plon =266.0
   plat = 41.0
   rnum = 1
...
# found in the CaseDocs/lnd_in file (note: fdomain is fatmlndfrc and fsurf is fsurdat)

fdomain = '/glade/p/cesmdata/cseg/inputdata/share/domains/domain.lnd.fv1.9x2.5_gx1v6.090206.nc'
fsurf = '/glade/p/cesmdata/cseg/inputdata/lnd/clm2/surfdata_map/surfdata_1.9x2.5_16pfts_simyr2000_c160127.nc'
# flanduse = '/glade/p/cesmdata/cseg/inputdata/lnd/clm2/surfdata_map/landuse.timeseries_0.9x1.25_rcp8.5_simyr1850-2100_c141219.nc' DLL: commented because not using

# specify DATM domain and forcing dataset; this information can be
# found in the CaseDocs/datm.streams.txt* files

dir_input_datm='/glade/p/cesm/lmwg/atm_forcing.datm7.cruncep_qianFill.0.5d.V4.c130305'
fdatmdomain = '/glade/p/cesmdata/cseg/inputdata/share/domains/domain.clm/domain.lnd.360x720.130305.nc'

solrdir  = 'Solar6Hrly/'
precdir  = 'Precip6Hrly/'
tpqwldir = 'TPHWL6Hrly/'

solrtag  = 'clmforc.cruncep.V4.c2011.0.5d.Solr.'
prectag  = 'clmforc.cruncep.V4.c2011.0.5d.Prec.'
tpqwtag  = 'clmforc.cruncep.V4.c2011.0.5d.TPQWL.'

# specify locations for output; must change this to a directory to 
# which you have permissions

dir_output     ='/glade/scratch/dll/SinglePointInputData'
dir_output_datm='/glade/scratch/dll/SinglePointInputData'

# specify new name for CLM domain file and surface data file
fdomain2  = dir_output+'domain.lnd.fv1.9x2.5_'+tag+'.nc'
fsurf2    = dir_output+'surfdata_1.9x2.5_'+tag+'.nc'
#flanduse2 = dir_output+'landuse.timeseries_0.9x1.25_rcp8.5_simyr1850-2100_'+tag+'.nc' DLL: commented because not using
