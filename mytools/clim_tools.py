...
def compute_vmr(data, **karg):
    ...
    Mtracer = karg.pop('molw',  compute_molarweight(data).sel(name=tracer)) # g/mol
...
def vapor_pressure(temperature, **karg):
    '''
    ...
    if version == 'buck':
        ...
        pressure = coeff_a * np.exp((coeff_b-temperature/coeff_c)*(temperature/(coeff_d+temperature)))
    elif version == 'campbell':
        coeff_a =   0.611   # kPa
        coeff_b =  17.502   # 1
        coeff_c = 240.97    # deg C
        pressure = coeff_a * np.exp(coeff_b*temperature / (temperature+coeff_c))
    elif version == 'rogers':
        coeff_a =   0.6112  # kPa
        coeff_b =  17.67    # 1
        coeff_c = 243.5     # deg C
...
def VPD(relhum, temperature, **karg):
    ...
    '''
    Ps = vapor_pressure(temperature, **karg)
    vpd = Ps * (1-relhum/100)
    # Return VPD in Pa
    return(vpd)
...
def dew_point(relhum, temperature, **karg):
    ...
    gamma_m = np.log(relhum/float(100)*pressure/coeff_a)
...
def USstdP(height):
    '''
    Compute USstd atmosphere 1976: P(h)
    Prarameters
    -----------
    height : float
        in units of km
...
def weighted_mean_std(variances, weights, axis=0):
    '''
    Calculate the standard deviation of a weighted mean (np.average).
    Parameters
    ----------
    variances : float
        array
    ...
    --------
    axis : int
        Axis which shall be used. default : 1
